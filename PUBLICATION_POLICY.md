...
### Mandatory bilingual document notice

`PUBLIC-DOC-NOTICE-001` is the canonical public-document rule. Every Markdown document must display the approved English/Korean NOTE and CAUTION block directly below its title. New documents must not be merged until the block, current baseline, correct relative Disclaimer link, and product-change retest statement are verified.

Supporting HTML, SVG, image, and other artifacts must display the notice when practical. If the format cannot carry readable notice text, the containing Markdown document must provide the notice and identify the artifact as sanitized supporting evidence.
